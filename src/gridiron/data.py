...
def load_rosters_weekly(seasons: Iterable[int], refresh: bool = False) -> pl.DataFrame:
    """Week-by-week rosters.

    Use this, not ``load_rosters``, when you need the roster as of a
    specific week. The season-level ``load_rosters`` table carries a
    ``week`` column that holds the *last* week a player was rostered, not a
    per-week snapshot — filtering it for ``week == 1`` silently returns
    almost nothing.
    """
    import nflreadpy as nfl

    yrs = _seasons(seasons)
    key = f"rosters_weekly_{yrs[0]}_{yrs[-1]}"
    return cached(key, lambda: nfl.load_rosters_weekly(seasons=yrs), refresh)
